...
In the following two exercises, you'll practice editing raster data and visualizing it clearly and creatively.
You'll learn how to cut rasters based on administrative boundaries, combine multiple rasters into one view, and explore how to style maps with simple but powerful base R tools.
...
::: callout-note
## `r emo::ji("weight_lifting_man")` 1

Choose another raster layer from the `./data/z22/` folder — either a `.tif` or a `.nc` file.
Your task is to split the raster into two parts: one for the northern and one for the southern part of Germany!

Use the shapefile `VG250_LAN.shp` (located in `./data/`) to help define the boundaries:
This shapefile contains the outlines of Germany’s federal states (Bundesländer).
...
You can split the federal states into "North" and "South" — but keep in mind, some states are geographically in between ("gray zone").
...
Feel free to decide for yourself how you classify the "gray zone" states!
Your definition is part of the creative freedom in this exercise.
...
Now that you've split the raster into Northern and Southern Germany, let’s visualize your work!
Your task:
