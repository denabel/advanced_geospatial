...
## Cropping the population data

Now, cropping is easy using the `terra::crop()` function.

```{r}
#| eval: false
pop_grid_ger_2020 <- terra::rast("./data/deu_ppp_2020_1km_Aggregated.tif")

pop_grid_ger_2020_crop <- terra::crop(pop_grid_ger_2020, bbox_east)
...
```{r}
#| fig.asp: .8
terra::plot(pop_grid_ger_2020_crop)
```
:::
::::
...
#| eval: false
#| fig.asp: .8
pop_grid_ger_2020_mask <- 
  terra::mask(
    pop_grid_ger_2020, 
    terra::vect(east_german_states) # !
  )

plot(pop_grid_ger_2020_mask)
```
:::
...
plot(pop_grid_ger_2020_mask_crop)
```
:::
...
```{r}
terra::extract(pop_grid_ger_2020, points, ID = FALSE) |> 
  head(10)
```
...
```{r}
points$pop <- terra::extract(pop_grid_ger_2020, points, ID = FALSE)
...
Population counts for the whole of Germany helps us to identify urban and rural clusters. But there may be applications where we need to zoom in on a specific city to identify within-city variations. Let's do that for the German capital Berlin. For this purpose, we subset the district data and also mask and crop our population counts.
...
Next, for our density estimation we simply want to count points in raster grid cells. However, our point are unfortunately a bit sparse compared to our comprehensive population raster dataset. We cannot rely on 1 km² grid cells initially as with the population grid data. But 5 km² may be a good compromise. Let's do that!
...
Now, you may think this is disappointing. These data looks... not good. But fear not, working with raster data is powerful, as we now use a function you already now for projecting one CRS into another: `terra::project()`. This function can also be used to aggregate and disaggregate data based on the structure of another dataset. So, while we were not able to use 1 km² grid cells for our density 'estimation' initially, we can reproject our 5 km² onto a 1 km² grid like our population grid. A bit of masking also helps in getting rid of cells that are not within the border of Berlin
...
points_berlin_density <-
  points_berlin_density |> 
  terra::project(pop_berlin) |> 
  terra::mask(pop_berlin)
...
## Applying it to the population grid data
...
pop_berlin_edges <- sobel(pop_berlin_smooth)
```
...
points_now_raster <- 
  ...
  terra::rasterize(
    raster_target_layer, 
    field = "deu_ppp_2020_1km_Aggregated", 
    fun = "mean",
    background = 0
  )
