...
```{r}
#| echo: false
# plot(german_states["hello_world"])
...
We can craft a toy example to demonstrate spatial joins using the synthetic survey geocoordinates. This is how they look.
...
In this course, we only use a subset of the options for spatial joins in `sf`. For more please refer to the official [sf cheat sheet](https://rstudio.github.io/cheatsheets/sf.pdf).

![](../img/sf_cheatsheet.jpg){.r-stretch fig-align="center"}

## Finally: Transforming the CRS

Depending on our data sources, we have to change our CRS fairly often. That's easily doable in `sf` using the `sf::st_transform()`. Here's a closer look.
...
Let's load another dataset for comparing structures, because that's really easy in `terra`.

```{r}
#| eval: false
pop_grid_ger_2000 <- terra::rast("./data/deu_ppp_2000_1km_Aggregated.tif")
...
```{r}
terra::global(pop_grid_ger_2000, fun = "mean", na.rm = TRUE)
```
...
```{r}
all_raster_values <- terra::values(pop_grid_ger_2000)
...
::: {.column width="50%"}
```{r}
pop_grid_ger_2000_2020 <-  
  pop_grid_ger_2020 - pop_grid_ger_2000
...
#| echo: false
#| fig.asp: .8
terra::plot(pop_grid_ger_2000_2020)
```
:::
::::
...
```{r}
#| eval: false
pop_grid_ger_2020_stars <- 
  stars::read_stars("./data/deu_ppp_2020_1km_Aggregated.tif")
...
```{r}
terra::crs(pop_grid_ger_2020, describe = TRUE)$code
```

Let's pretend we want to use the EPSG code `25832`--no problem!

```{r}
pop_grid_ger_2020_25832 <- terra::project(pop_grid_ger_2020, "EPSG:25832")

terra::crs(pop_grid_ger_2020_25832, describe = TRUE)$code
```